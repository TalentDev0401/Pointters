...
protocol SendRequestOfferDelegate {
...
protocol SendCustomOfferDelegate {
...
class SendOfferViewController: UIViewController {
    ...
    var requestOfferDelegate: SendRequestOfferDelegate?
    ...
    var requestOfferDict: [String:Any]?
        ...
        ApiHandler.callGetRequestOfferDetail(offerId: offerId, withCompletionHandler:{ (result,statusCode,response, error) in
            ...
            if result == true {
                ...
                if statusCode == 200 {
                    ...
                    self.linkService = Service.init(dict: offerDetailDict)
                    self.isLinkService = true
                    ...
                    ApiHandler.deleteJobOffer(offerId: self.offerId) { (status, statusCode, response, error) in
                        ...
                        if status == true {
                            ...
                            if statusCode == 200 {
                                ...
                                if self.customOfferDelegate != nil {
                                    self.requestOfferDelegate?.returnFromRequestOffers(reload: true)
                                }
                                self.navigationController?.popViewController(animated: true)
                            } else {
                                PointtersHelper.sharedInstance.showAlertViewWithTitle("", message: responseDict["message"] as! String, buttonTitles: ["OK"], viewController: self, completion: nil)
        ...
        ApiHandler.callCreateJobOffer(jobId: requestId, sellerId: loginUserId, buyerId: buyerId, serviceId: linkService.id, service: self.linkService, currencyCode: numberFormatter.locale.currencyCode!, currencySymbol: numberFormatter.locale.currencySymbol!, description: offerDescription, fulfillmentMethod: fullfillmendMethod, price: customPrice, workDuration: Float(self.price.time), workDurationUom: self.price.timeUnitOfMeasure, address: address, parcel: parcel) { (status, statusCode, response) in
            ...
            if status == true {
                ...
                if statusCode == 200 {
                    ...
                    price["workDuration"] = offerDict["workDuration"] as! Int
                    price["workDurationUom"] = offerDict["workDurationUom"] as! String
                    let service: [String:Any] = offerDict
                    self.requestOfferDelegate?.selectSendRequestOffers(selId: offerDict["_id"] as! String, selPrice: price, linkedService: service, link: self.isLinkService)
                    for controller in self.navigationController!.viewControllers as Array {
                        if controller.isKind(of: PrivateChatViewController.self) {
...
extension SendOfferViewController:LinkServiceDelegate {
...
extension SendOfferViewController : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            if indexPath.row == 0 {
                ...
                cell.lblPrice.text = self.linkService.prices.currencySymbol + "$6"//String(format: "%.2f", self.linkService.prices.price)
...
extension SendOfferViewController: ShippingDetailDelegate {
...
extension SendOfferViewController: SettingRadiusDelegate {
...
extension SendOfferViewController: UIPickerViewDataSource {
...
extension SendOfferViewController: UIPickerViewDelegate {
...
extension SendOfferViewController: CLLocationManagerDelegate {
