//
//  ChatSendRequestCell.swift
//  Pointters
//
//  Created by Billiard ball on 24.06.2020.
//  Copyright © 2020 Kenji. All rights reserved.
//

import UIKit

class ChatSendRequestCell: ChatBaseCell {
...
//            self.lblName.textColor = UIColor.darkGray
        ...
        if let dictRequest = message["request"] as? [String:Any] {
            if let dictMedia = dictRequest["media"] as? [String:Any] {
            ...
            if let strTitle = dictRequest["serviceName"] as? String, strTitle != "" {
                self.lblServiceDesc.text = strTitle
            }
            
            if let strDesc = dictRequest["description"] as? String, strDesc != "" {
                self.lblServiceWork.text = strDesc
            }
            
            if let dictSeller = dictRequest["seller"] as? [String:Any] {
                var strFirst = ""
